...
export const BookshelfMenu = ({ onBookClick, visibleSections, currentYoungAdultBook = 0 }: BookshelfMenuProps) => {
  ...
  return (
    <div className="sticky top-16 z-20 bg-black/80 backdrop-blur-md border-b border-white/20 py-8 max-sm:py-3">
      <div className="container mx-auto px-6 max-sm:px-4">
        <div className="flex justify-center items-center gap-6 max-sm:gap-4 overflow-x-auto pb-2 max-sm:pb-1 max-sm:justify-start" style={{ scrollSnapType: 'x mandatory' }}>
          {books.map((book) => (
            <div
              key={book.id}
            ...
            >
              {/* Book Title */}
              <h3 className={`font-serif text-xs max-sm:text-[10px] font-semibold mb-1 text-center group-hover:text-yellow-300 transition-colors duration-300 whitespace-nowrap ${
              ...
              <div className="relative">
                <img
                  ...
                  height="64"
                  loading="eager"
                  className={`w-16 h-16 max-sm:w-12 max-sm:h-12 rounded shadow-lg transition-all duration-300 group-hover:shadow-xl object-cover ${
                    activeBook === book.id
                      ? 'scale-110 max-sm:scale-105 shadow-xl shadow-yellow-300/30 ring-2 ring-yellow-300/50'
                      : hoveredBook === book.id 
                        ? 'scale-125 max-sm:scale-110 shadow-2xl shadow-yellow-300/20' 
                        : 'hover:scale-110 max-sm:hover:scale-105'
                  }`}
                />
