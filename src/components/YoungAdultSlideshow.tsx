...
export const YoungAdultSlideshow = forwardRef<YoungAdultSlideshowRef, YoungAdultSlideshowProps>(({ onBookChange }, ref) => {
  ...
  return (
    <div className="relative w-full bg-black/60 backdrop-blur-sm rounded-lg overflow-hidden shadow-lg border border-white/20">
      <div className="relative px-20 py-8 max-sm:px-4 max-sm:py-6 pb-16 max-sm:pb-20">
        <div className={`grid grid-cols-1 lg:grid-cols-2 gap-8 max-sm:gap-4 items-center ${
          book.layout === "cover-right" ? "lg:grid-flow-col-dense" : ""
        }`}>
          {/* Book Cover */}
          <div className={`${book.layout === "cover-right" ? "lg:col-start-2" : ""} max-sm:order-1`}>
            <img 
              key={`cover-${currentBook}`}
              src={book.cover} 
              alt={book.title}
              className="w-full max-w-xs max-sm:max-w-[200px] mx-auto object-contain rounded-lg shadow-lg transition-opacity duration-100"
              loading="eager"
            />
          </div>
          
          {/* Book Info */}
          <div className={`${book.layout === "cover-right" ? "lg:col-start-1 pl-24 pr-8" : "pr-24 pl-8"} max-sm:px-0 max-sm:order-2`}>
            <h3 className="font-serif text-3xl max-sm:text-xl font-bold text-white mb-2 drop-shadow-lg">{book.title}</h3>
            {book.subtitle && (
              <h4 className="font-serif text-xl max-sm:text-base text-yellow-300 mb-4 max-sm:mb-2 drop-shadow-lg">{book.subtitle}</h4>
            )}
            <p className="font-serif text-lg max-sm:text-sm leading-relaxed text-white/90 drop-shadow-md">
              {book.summary}
            </p>
          </div>
        </div>
      ...
      {/* Navigation */}
      <div className="absolute left-4 max-sm:left-2 top-1/2 max-sm:top-auto max-sm:bottom-2 transform -translate-y-1/2 max-sm:translate-y-0">
        <Button
          variant="outline"
          size="lg"
          onClick={prevBook}
          className="bg-black/60 border-2 border-yellow-300/60 text-yellow-300 hover:bg-black/80 hover:border-yellow-300 hover:scale-110 transition-all duration-300 rounded-full w-12 h-12 max-sm:w-10 max-sm:h-10 p-0 backdrop-blur-sm shadow-xl"
        >
          <ChevronLeft className="w-6 h-6 max-sm:w-5 max-sm:h-5" />
        </Button>
      </div>
      <div className="absolute right-4 max-sm:right-2 top-1/2 max-sm:top-auto max-sm:bottom-2 transform -translate-y-1/2 max-sm:translate-y-0">
        <Button
          variant="outline"
          size="lg"
          onClick={nextBook}
