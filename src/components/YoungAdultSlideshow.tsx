...
export const YoungAdultSlideshow = forwardRef<YoungAdultSlideshowRef, YoungAdultSlideshowProps>(({ onBookChange }, ref) => {
  ...
  return (
    <div className="relative w-full bg-black/60 backdrop-blur-sm rounded-lg overflow-hidden shadow-lg border border-white/20">
      <div className="relative px-20 py-8 max-md:px-4 max-md:py-6 pb-16 max-md:pb-20">
        <div className={`grid grid-cols-1 lg:grid-cols-2 gap-8 max-md:gap-4 items-center ${
          book.layout === "cover-right" ? "lg:grid-flow-col-dense" : ""
        }`}>
          {/* Book Cover */}
          <div className={`${book.layout === "cover-right" ? "lg:col-start-2" : ""} max-md:order-1`}>
            <img 
              key={`cover-${currentBook}`}
              src={book.cover} 
              alt={book.title}
              className="w-full max-w-xs max-md:max-w-[200px] mx-auto object-contain rounded-lg shadow-lg transition-opacity duration-100"
              loading="eager"
            />
          </div>
          
          {/* Book Info */}
          <div className={`${book.layout === "cover-right" ? "lg:col-start-1 pl-24 pr-8" : "pr-24 pl-8"} max-md:px-0 max-md:order-2`}>
            <h3 className="font-serif text-3xl max-md:text-xl font-bold text-white mb-2 drop-shadow-lg">{book.title}</h3>
            {book.subtitle && (
              <h4 className="font-serif text-xl max-md:text-base text-yellow-300 mb-4 max-md:mb-2 drop-shadow-lg">{book.subtitle}</h4>
            )}
            <p className="font-serif text-lg max-md:text-sm leading-relaxed text-white/90 drop-shadow-md">
              {book.summary}
            </p>
          </div>
        </div>
      ...
      {/* Navigation */}
      <div className="absolute left-4 max-md:left-2 top-1/2 max-md:top-auto max-md:bottom-2 transform -translate-y-1/2 max-md:translate-y-0">
        <Button
          variant="outline"
          size="lg"
          onClick={prevBook}
      ...
      <div className="absolute right-4 max-md:right-2 top-1/2 max-md:top-auto max-md:bottom-2 transform -translate-y-1/2 max-md:translate-y-0">
        <Button
          variant="outline"
          size="lg"
          onClick={nextBook}
